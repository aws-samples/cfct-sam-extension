...
def resolve(path, obj, separatror = "."):
    for element in path.split("."):
        if element in obj:
            nested_object = obj.get(element)
            obj = nested_object
        else:
            raise Exception('Error: ', element, 'is not a valid key to get resource parameters')
    return obj
...
def evaluate_change_notification_compliance(configuration_item, rule_parameters):
    ...
    if (configuration_item['resourceType'] != rule_parameters['resourceType']):
        return 'NOT_APPLICABLE'

    elif rule_parameters.get('keyPath'):
        if resolve(rule_parameters["keyPath"], configuration_item['configuration']) \
           in rule_parameters['acceptedValues']:
            return 'COMPLIANT'
